...
class _AutoPlayListState extends State<AutoPlayList> {
  ...
  void _listener(){
    ...
    if(_scrollController.hasClients){
      ...
      if(_lastScrollPosition - 10 <= _scrollController.position.pixels || _scrollController.position.pixels <= _lastScrollPosition + 10){
        _isScrolling = false;
        _notifier.setIndex((_scrollController.position.pixels / (widget.itemExtent)).ceil());
      } 
