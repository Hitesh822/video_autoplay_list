...
class AutoPlayVideo extends StatefulWidget {
  ...
  AutoPlayVideo({
    ...
    :
    assert(network != null || asset != null || file != null, 'One of the datasource (network, asset, file) must not be null.'),
    assert((network == null && asset == null) || (network == null && file == null) || (file == null && asset == null), 'Only one of the datasource (network, asset, file) can be provided.')
  ;
...
class _AutoPlayVideoState extends State<AutoPlayVideo> with SingleTickerProviderStateMixin{
  ...
  void initState() {
    ...
    _colorAnimation = Tween<Color>(begin: Colors.red, end: Colors.red).animate(CurvedAnimation(parent: _animationController, curve: Curves.easeIn));
    if(widget.network != null){
      ...
      bool isWeb;
      try{
        if(Platform.isAndroid) isWeb = false;
        else if(Platform.isIOS) isWeb = false;
        else isWeb = true;
      }
      catch(e){
        isWeb = true;
  ...
  Widget build(BuildContext context) {
    ...
    if(widget.index != null && _controller.value.initialized && widget.autoPlay && _notifier.nextVideoIndex.value == widget.index && !_controller.value.isPlaying && !_onceAutoplayed){
      _onceAutoplayed = true;
      _controller.play();
    }
    else if(widget.index != null && _notifier.nextVideoIndex.value != widget.index && _controller.value.initialized && _controller.value.isPlaying){
      // _onceAutoplayed = false;
      _controller.pause();
    }
          ...
          children: [
            InkWell(
              ...
              : widget.loader ?? Container(
                color: Colors.black, 
                child: Center(
                  child: CircularProgressIndicator(backgroundColor: Colors.white.withOpacity(0.2), valueColor: _colorAnimation,),
                ),
              ),
            ),
      ...
      child: Column(
        children: [
          ...
          Row(
            children: [
              // current duration
              Text(_formatDuration(_controller.value.position, false), style: TextStyle(color: Colors.white, fontSize: 12),),
              SizedBox(width: 6,),
              // duration progress indicator
              Expanded(child: LinearProgressIndicator(value: _durationRatio(), backgroundColor: Colors.white.withOpacity(0.2), minHeight: 2, valueColor: _colorAnimation)),
              SizedBox(width: 6,),
              // total duration
              Text(_formatDuration(_controller.value.duration, true), style: TextStyle(color: Colors.white, fontSize: 12),),
            ],
          ),
        ],
      ),
  ...
  String _formatDuration(Duration duration, bool isTotalDuration){
    ...
    if(_controller.value.duration.inHours > 0){
      if(hours < 9) str += '0$hours:';
      else str += '$hours:';
    }
    
    if(minutes < 9) str += '0$minutes:';
    else str += '$minutes:';

    if(seconds < 9) str += '0$seconds';
    else str += '$seconds';
  ...
  double _durationRatio(){
    ...
    return _controller.value.position.inSeconds / _controller.value.duration.inSeconds;
  }
